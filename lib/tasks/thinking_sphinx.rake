...
namespace :thinking_sphinx do
  task run_in_foreground: ['ts:stop', 'ts:configure', 'ts:index'] do
    ...
    #   - Which turns SIGTERM into SIGHUP
    #     (not sure exactly why, can't seem to find a reason)
    #   - Which sphinx interprets as a reload instead of a quit
    #   - So, we need to remap HUP to KILL for the purposes of this script.

    unless defined? pid
      exec "#{controller.bin_path}\
            #{controller.searchd_binary_name} --pidfile --config \
            #{controller.path} --nodetach"
    end
    pid ||= fork
